  ...
  return (
    <span
      className={cn(
        ...
        STATUS_COLORS[status] ?? "text-slate-400 bg-white/5 border-white/10",
      )}
    >
      <span
        className={cn(
...
export function ConfidenceBar({
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      ...
      {showLabel ? (
        <span className="mono text-xs text-slate-300 tabular-nums">
          {pct(v)}
        </span>
      ) : null}
    </div>
...
export function StatCard({
  ...
  return (
    <Card className="p-4 relative overflow-hidden glass-hover">
      ...
      {delay != null ? (
        <div className="absolute bottom-0 left-0 right-0 h-px overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-transparent via-ghost/60 to-transparent"
  ...
  return (
    <div className="flex items-center gap-3 text-sm text-slate-400">
      <div className="relative w-6 h-6">
        <div
          className="absolute inset-0 rounded-full border border-ghost/40 animate-spin"
...
export function EmptyState({ title, hint }: { title: string; hint?: string }) {
  return (
    <div className="py-16 text-center">
      <div className="mx-auto mb-3 w-12 h-12 rounded-xl border border-white/10 bg-white/[0.02] flex items-center justify-center text-slate-600 text-xl">
        ∅
      </div>
...
export function Spinner({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "w-4 h-4 rounded-full border-2 border-ghost/30 border-t-ghost animate-spin",
        className,
      )}
    />
  );
...
export function Button({
  ...
  const styles: Record<string, string> = {
    ...
    default:
      "bg-base-800 hover:bg-base-700 text-slate-200 border border-white/10",
    danger:
  ...
  return (
    <button
      className={cn(
        "inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-lg text-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed",
        styles[variant],
        className,
      )}
      {...props}
